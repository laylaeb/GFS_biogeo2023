...
## Then we ungroup and we want to know how many ASVs are specific so we just group by ASV and count the nb of times an ASV is observed throughout the dataset 
...
## We would like create a stacked plot with the taxonomy of the three different fractions (unique, 2-9 and >10)
...
## filter only ASVs > 0 and we are intested in the median of relative abundance 
...
## Median of relative abundance of ASVs of a given mountain range
# # A tibble: 10 × 3
# site_c         med_asv     n
# <chr>            <dbl> <int>
