# Radar per region -- taking into account the median values for each parameter
...
## load the minerals
minerals_nomis <- read.csv("/Users/ezzat/Leilas_Drive/Projet SBER/NOMIS/NOMIS_16S/NOMIS_August2023/DEBLUR/minerals_nomis_2023.csv")
minerals_nomis <- minerals_nomis[,c(1:5)]

## load the climatic variabale as well!
climatic_nomis <- as.data.frame(read.csv("/Users/ezzat/Leilas_Drive/Projet SBER/NOMIS/NOMIS_16S/NOMIS_August2023/DEBLUR/climate_nomis_2023.csv"))
...
## Keep the data you want --  Here I decided to remove NO2 because too many 0
radar_data_subset <- metadata_radar_corrected_updown[c("gl_code","site_c","pH", "pH", "cond", "turb","doc","srp", "NH4","NO3", "sba", "chla","gl_sa","gl_cov","lat_sp.x","lon_sp.x","ele_sp","Feldspar","Clays","Calcite","Quartz","pr","sn_sp_dist","scd")]
## remove NA values
radar_data_subset_na <- na.omit(radar_data_subset) ## from 906 lines to 828

## Sum inorganic nitrogen into DIN
rowsum_nut <- rowSums(radar_data_subset_na[, c("NO3", "NH4")])
radar_data_subset_na$DIN <- rowsum_nut 

#################################### now it is pretty clean!
## we have to average values for each patch =)
# Select only the numeric columns (excluding lat_sp and lon_sp, gl_code)
numeric_columns <- setdiff(names(radar_data_subset_na), c("gl_code","site_c","lat_sp.x","lon_sp.x"))
...
metadata_radar_subset_table<- metadata_radar_sub_u_with_site[c('site_c','pH', 'pH', 'cond', 'turb', 'gl_sa', 'gl_cov', 'srp', 'DIN','doc','Feldspar','Calcite','Quartz','Clays','chla','sn_sp_dist','sba', 'pr','scd')]
metadata_radar_subset_plot<- metadata_radar_sub_u_with_site[c('site_c','pH', 'pH', 'cond', 'turb', 'gl_sa', 'gl_cov', 'srp', 'DIN','doc')]
...
process_site_data <- function(data, site_name) {
  ...
  # rename column
  colnames(q25_pivot) <- c("group","pH","pH","cond","turb","gl_sa","gl_cov","SRP","DIN","doc")
  colnames(q50_pivot) <- c("group","pH","pH","cond","turb","gl_sa","gl_cov","SRP","DIN","doc")
  colnames(q75_pivot) <- c("group","pH","pH","cond","turb","gl_sa","gl_cov","SRP","DIN","doc")
  
  # colnames(q25_pivot) <- c("group","pH","pH","cond","turb","gl_sa","gl_cov","SRP","DIN","Feldspar","Calcite","Quartz","Clays","mean_chla","sn_sp_dist")
  # colnames(q50_pivot) <- c("group","pH","pH","cond","turb","gl_sa","gl_cov","SRP","DIN","Feldspar","Calcite","Quartz","Clays","mean_chla","sn_sp_dist")
  # colnames(q75_pivot) <- c("group","pH","pH","cond","turb","gl_sa","gl_cov","SRP","DIN","Feldspar","Calcite","Quartz","Clays","mean_chla","sn_sp_dist")
  
  
  # Combine q25, q50, and q75 data
  min <- data.frame(matrix(rep(c(0), 10), nrow = 1))
  colnames(min)<-c("group","pH","pH","cond","turb","gl_sa","gl_cov","SRP","DIN","doc")
  #colnames(min)<-c("group","pH","pH","cond","turb","gl_sa","gl_cov","srp","DIN")
...
## replace just the name of the spr into SRP
...
gg_water <- ggplot(metadata_radar_sub_u_with_site, aes(x = site_c, y = pH, color = site_c)) +
  ...
  # Add lines for median, lower, and upper quartiles
  ...
  # Set the y-axis limits to show the full range of quartiles
  ylim(c(min(metadata_radar_sub_u_with_site$pH) - 1, max(metadata_radar_sub_u_with_site$pH) + 1)) +
  
  # Remove legend for both x and y axes
...
mean_value_wt <- mean(metadata_radar_sub_u_with_site$pH)
sd_value_wt <- sd(metadata_radar_sub_u_with_site$pH)
...
# mean_value_turb <- mean(metadata_radar_sub_u_with_site$turb)
# sd_value_turb <- sd(metadata_radar_sub_u_with_site$turb)
...
mean_value_DIN <- mean(metadata_radar_sub_u_with_site$DIN)
sd_value_DIN <- sd(metadata_radar_sub_u_with_site$DIN)
...
mean_value_DOC <- mean(metadata_radar_sub_u_with_site$doc)
sd_value_DOC <- sd(metadata_radar_sub_u_with_site$doc)
...
mean_value_chla <- mean(metadata_radar_sub_u_with_site$chla)
sd_value_chla <- sd(metadata_radar_sub_u_with_site$chla)
...
gg_Clays <- ggplot(metadata_radar_sub_u_with_site, aes(x = site_c, y = Clays, color = site_c)) +
  ...
  xlab("Site") +
  ylab("Clays")
mean_value_Clays <- mean(metadata_radar_sub_u_with_site$Clays)
sd_value_Clays <- sd(metadata_radar_sub_u_with_site$Clays)
...
gg_Calcite <- ggplot(metadata_radar_sub_u_with_site, aes(x = site_c, y = Calcite, color = site_c)) +
  ...
  xlab("Site") +
  ylab("Calcite")
mean_value_Calcite <- mean(metadata_radar_sub_u_with_site$Calcite)
sd_value_Calcite <- sd(metadata_radar_sub_u_with_site$Calcite)
...
gg_Quartz <- ggplot(metadata_radar_sub_u_with_site, aes(x = site_c, y = Quartz, color = site_c)) +
  ...
  xlab("Site") +
  ylab("Quartz")
mean_value_Quartz <- mean(metadata_radar_sub_u_with_site$Quartz)
sd_value_Quartz <- sd(metadata_radar_sub_u_with_site$Quartz)
...
mean_value_Feldspar <- mean(metadata_radar_sub_u_with_site$Feldspar)
sd_value_Feldspar <- sd(metadata_radar_sub_u_with_site$Feldspar)
...
gg_gl_sa <- ggplot(metadata_radar_sub_u_with_site, aes(x = site_c, y = gl_sa, color = site_c)) +
  ...
  xlab("Site") +
  ylab("gl_sa")
mean_value_gl_sa <- mean(metadata_radar_sub_u_with_site$gl_sa)
sd_value_gl_sa <- sd(metadata_radar_sub_u_with_site$gl_sa)
...
gg_gl_cov <- ggplot(metadata_radar_sub_u_with_site, aes(x = site_c, y = gl_cov, color = site_c)) +
  ...
  xlab("Site") +
  ylab("gl_cov")
mean_value_gl_cov <- mean(metadata_radar_sub_u_with_site$gl_cov)
sd_value_gl_cov <- sd(metadata_radar_sub_u_with_site$gl_cov)

library(ggpubr)
ggarrange(gg_water,gg_pH,gg_cond,gg_turbidity)
