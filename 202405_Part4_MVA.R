...
## We start with rarefied data -- 152 GFSs and remove Uganda here as well
require(ggplot2)
require(reshape)
require(scales)
library(scales)
...
# Now we need to keep lat, lon and ele values from up samples only! if there are only down samples, then keep values from down (GL27,GL56 and 80 for genes, for instance!)
multi_data_subset_na_coordinates <- multi_data_subset_na %>%
  group_by(gl_code) %>%
...
# Calculate the average for each numeric column (not taking into account lat, lon, ele_sp and the categorical variables)
...
## only gl_cov distributed normally...so we will have to adjust and logtransform them!
...
##################STATs 
metadata_stat<-metadata_multi_sub_u_with_site

numeric_vars <- c("water_temp", "pH", "cond", "turb", "doc", "srp", "NH4", "NO3", "chla",
                  "gl_sa", "gl_cov", "pr", "scd", "Clays", "Quartz", "Calcite", "Feldspar", "DIN")
...
+
  ggplot2::scale_color_manual(values = c("#2E2A2BFF", "#CF4E9CFF", "#8C57A2FF", "#3EBCB6", "#82581FFF", "#2F509EFF", "#E5614CFF", "#97A1A7FF", "#bee183", "#DC9445FF", "#EDD03E"))
