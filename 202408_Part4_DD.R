...
## To include in the MS, we will have to subset the matrices because too heavy to include the figure
...
# Center the geo.dist.log variable
manco$geo.dist.log_centered <- scale(manco$geo.dist.log, center = TRUE, scale = FALSE)

# Fit the model with the centered variable
glm_dd_bcsor <- lm(sqrt(value) ~ geo.dist.log_centered * variable, data = manco)
check_model(glm_dd_bcsor)
summary(glm_dd_bcsor)

manco_unifrac$geo.dist.log <- log((manco_unifrac$geo.dist.x)/1000)
manco_unifrac$geo.dist.log_centered <- scale(manco_unifrac$geo.dist.log, center = TRUE, scale = FALSE)

glm_dd_wufrac <- lm(manco_unifrac, formula = value ~ geo.dist.log_centered*variable)
