...
indicator_data <- as.data.frame(sample_data(prune_Uganda))
asv_prune_uganda <- otu_table(prune_Uganda, taxa_are_rows=T)
asv_prune_uganda_filtered <- asv_count_level[rowSums(asv_count_level[])>0,]

ra_prune_uganda<- transform_sample_counts(prune_Uganda, function(x) x / sum(x))
ra_asv_prune_uganda <- otu_table(asv_indicator_ab, taxa_are_rows=T)
ra_asv_prune_uganda_filtered <- asv_indicator[rowSums(asv_indicator[])>0,]

## Relative abundance of the indicator taxa
## asv_indicator -> table d'abondance relative 
ra_indicator <- ra_asv_prune_uganda[row.names(ra_asv_prune_uganda) %in% (indicator_taxa$X)]
...
merge_asv_indicator <- merge(as.data.frame(melt_asv_indicator),as.matrix(metadata_NOMIS), by.x="Var2",by.y="sample")
...
sum_mr_indicator <- merge_asv_indicator %>% group_by(site_c)%>% summarize(summrr=sum(value), n=n_distinct(Var2))%>% summarize(ar_mr=summrr/n, site_c)
