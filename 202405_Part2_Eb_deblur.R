...
##### Endemism
## up samples only - Liste des ASVs qui ont une prévalence de 1 : 11231 soit 28.4% -- comment sont-ils répartis par mountain ranges?
## 2024 - up + down - Liste des ASVs qui ont une prévalence de 1: 13594 soit 13594/54019=25.2%
endemic_oneGFS <- howmanyasv %>% 
  filter(Count_nb == 1) 
write.csv(endemic_oneGFS, "2024_unique_list.csv")
## careful there may be some issues with package dependencies..
...
endemic_MR_prev <- merge_equalone %>% group_by(MR) %>% summarize(prev=sum(prev)) ## nb of unique ASV per mountain range
...
## Proportion of unique ASVs per mountain range!
####2024 results
# MR           prop$n
# <chr>         <dbl>
# 1 Alaska       0.125 
...
# 5 Ecuador      0.134 
# 6 Greenland    0.0206
# 7 Kirghizistan 0.107 
# 8 Nepal        0.139 
# 9 New_Zealand  0.0984
# 10 Norway      0.0367
...
## en gros on compte le nombre de lignes par ASV et Glcode. Mais ça ne veut pas dire qu'il est unique à cette région, ça sera l'étape suivante...
twoandnine_end<- merge_twoandnine %>% group_by(MR,ASV) %>% summarize(prev=n()) 
twoandnine_end$Color <- "B_twoandnine"

## Maintenant on veut qu'il soit unique dans la mountain range! On  mutate pour pouvoir compter le nombre de fois ou l'ASV est observé (dans combien de MR), puis on filtre seulement les ASVs qui sont présents dans une seule MR
...
## Maintenant la proportion par MR
...
###2024 -- 648/54019=1.2% of total ASVs are endemic to this section (>=10GFs)
...
###33617 that's the sum. 33617/54019=62.2% ## this works with previous calculations!
...
##Proportion of endemic ASVs per mountain range
...
## We would like create a stackedplot with the taxonomy of the three different fractions (unique, 2-9 and >10)
...
## Ceci représente la médiane et autre IQR de la moyenne des contributions des ASVs en terme d'abondance relative par région
## Data 2024
# # A tibble: 3 × 2
# median      x
# <dbl>  <dbl>
...
## ici on filtre uniquement les ASVs qui ont une > 0 et on s'intéresse à la médiane de leur abondance relative
...
## Ici ça correspond simplement à la mediane de l'abondance relative des ASVs de la région en particulier 
# # A tibble: 10 × 3
# site_c         med_asv     n
# <chr>            <dbl> <int>
